<template>
    <AppLayout title="Dashboard Líder" subtitle="Visão dos setores sob sua responsabilidade">
        <div class="space-y-6">

            <!-- Scope info banner -->
            <div v-if="permissoes.length > 0" class="card p-4 bg-indigo-50 border border-indigo-200">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-indigo-800 mb-1">Escopo de acesso</p>
                        <div class="flex flex-wrap gap-2">
                            <span
                                v-for="(p, i) in permissoes"
                                :key="i"
                                class="inline-flex items-center px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700 text-xs font-medium"
                            >
                                <span v-if="p.unidade">{{ p.unidade }}</span>
                                <span v-if="p.unidade && p.setor" class="mx-1 text-indigo-400">›</span>
                                <span v-if="p.setor">{{ p.setor }}</span>
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div v-else class="card p-6 bg-yellow-50 border border-yellow-200 text-center">
                ...
                <p class="text-sm font-semibold text-yellow-800">Nenhuma permissão atribuída</p>
                <p class="text-xs text-yellow-600 mt-1">Solicite ao RH que configure seu acesso aos setores.</p>
            </div>

            <!-- Filter bar -->
            <div v-if="unidades.length > 0 || setores.length > 0" class="card p-4">
                <form @submit.prevent="aplicarFiltros" class="flex flex-wrap items-end gap-4">
                    <div v-if="unidades.length > 1" class="flex-1 min-w-[180px]">
                    ...
                    <div v-if="setores.length > 1" class="flex-1 min-w-[180px]">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div class="card p-4">
                    ...
                    <p class="text-2xl font-bold text-blue-600">{{ metricas.taxa_adesao != null ? metricas.taxa_adesao + '%' : '—' }}</p>
                </div>
                <div class="card p-4">
                    <p class="text-xs font-medium text-gray-500 mb-1">IGRP</p>
                    <p class="text-2xl font-bold text-purple-600">{{ metricas.igrp != null ? metricas.igrp + '%' : '—' }}</p>
                </div>
                <div class="card p-4">
                    <p class="text-xs font-medium text-gray-500 mb-1">Risco Alto</p>
                    <p class="text-2xl font-bold text-red-600">{{ metricas.percentual_risco_alto != null ? metricas.percentual_risco_alto + '%' : '—' }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="card p-6">
                    ...
                    <p class="text-xs text-gray-400 mb-4">Top setores por risco médio (no seu escopo)</p>
                    <div class="h-72">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="card p-6">
                    ...
                    <p class="text-xs text-gray-400 mb-4">Dimensões negativas em vermelho</p>
                    <div class="h-72">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="card p-6">
                    ...
                    <p class="text-xs text-gray-400 mb-4">Comparativo entre gêneros por dimensão</p>
                    <div class="h-72">
                ...
                <div class="card p-6">
                    ...
                    <p class="text-xs text-gray-400 mb-4">Comparativo entre faixas etárias por dimensão</p>
                    <div class="h-72">
            ...
            <div class="card p-6">
                ...
                <p class="text-xs text-gray-400 mb-4">Índice de risco (%) por setor e dimensão.</p>
...
const radarOptions = { responsive: true, maintainAspectRatio: false, scales: { r: { min: 0, max: 4 } }, plugins: { legend: { display: false } } };
...
const setoresBarData = computed(() => ({
    ...
    datasets: [{
        ...
        backgroundColor: setoresCrit.value.map(s => s.risco_medio >= 75 ? '#ef4444' : s.risco_medio >= 50 ? '#f97316' : s.risco_medio >= 25 ? '#f59e0b' : '#22c55e'),
    }],
}));
const barHorizOptions = { indexAxis: 'y', responsive: true, maintainAspectRatio: false, scales: { x: { min: 0, max: 100 } }, plugins: { legend: { display: false } } };
...
const barHorizDimOptions = { indexAxis: 'y', responsive: true, maintainAspectRatio: false, scales: { x: { min: 0, max: 4 } }, plugins: { legend: { display: false } } };
...
const barVertOptions = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } };
...
const lineOptions = { responsive: true, maintainAspectRatio: false, scales: { y: { min: 0, max: 4 } }, plugins: { legend: { position: 'top' } } };
